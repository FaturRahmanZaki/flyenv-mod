<template>
  ...
  >
    <div class="aside-inner">
      ...
      <div v-show="appStore.asideExpanded" class="aside-search px-3 pb-2">
        <el-input
      ...
      <el-scrollbar>
        <ul class="menu top-menu">
          <template v-for="(item, _index) in asideModule" :key="_index">
            <div
              :style="
                {
...
<script lang="ts" setup>
  ...
  const asideModule = computed(() => {
    const keyword = searchKeyword.value.trim().toLowerCase()
    if (!appStore.asideExpanded || !keyword) {
      return allModule.value
    }
    return allModule.value
      .map((item: any) => {
        const categoryLabel = `${item?.label ?? ''}`.toLowerCase()
        const sub = item.sub.filter((m: any) => {
          const label = getModuleLabel(m).toLowerCase()
          const typeFlag = `${m?.typeFlag ?? ''}`.toLowerCase()
          const id = `${m?.id ?? ''}`.toLowerCase()
          return (
            label.includes(keyword) ||
            typeFlag.includes(keyword) ||
            id.includes(keyword) ||
